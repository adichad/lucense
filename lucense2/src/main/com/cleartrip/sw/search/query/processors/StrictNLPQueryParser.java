...
public class StrictNLPQueryParser extends CustomQueryParser {
  ...
  public StrictNLPQueryParser(Version version, Analyzer an,
  ...
  private static final class BasicFieldQueryFiller extends FieldQueryFiller {
    ...
    boolean fillToken(String token, Set<String> fillFields, int pos,
      ...
      if (!fuzzy) {
        for (Term t : baseTerms) {
          if (fillFields.contains(t.field())) {
            qf.add(bq, qf.newTermQuery(t.createTerm(token), pos), occur);
          }
          filled = true;
        }
      } else {
        ...
        for (Term t : baseTerms) {
          if (fillFields.contains(t.field())) {
            terms.add(t.createTerm(token));
          }
        }

        List<Query> queries = new LinkedList<>();
        qf.newTermXorFuzzyQueries(queries, reader, terms,
            fuzzySimilarity <= 0.01f ? getMinSimilarity(token)
                : fuzzySimilarity, 2, pos, fuzzyThreshold);
        for (Query q : queries) {
          qf.add(bq, q, occur);
          filled = true;
          //
  ...
  public Query parseStructured(SearchParameters searchParams)
    ...
    StrictNLPSearchParameters params = (StrictNLPSearchParameters) searchParams;
