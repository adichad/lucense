...
abstract class CustomPhraseScorer extends CustomScorer {
  ...
  PhrasePositions  cmin, cmax;

  private float    freq;      // phrase frequency in current doc as computed by
                               // phraseFreq().

  CustomPhraseScorer(Weight weight, CustomPhraseQuery.PostingsAndFreq[] postings,
      CustomPhraseQuery.PostingsAndFreq[] cpostings, Similarity similarity,
      byte[] norms) {
    super(similarity);
    this.norms = norms;
    ...
    if (postings.length > 0) {
      ...
      for (int i = 1; i < postings.length; i++) {
        PhrasePositions pp = new PhrasePositions(postings[i].postings,
            postings[i].position, i, postings[i].terms);
        max.next = pp;
        max = pp;
        max.doc = -1;
      }
