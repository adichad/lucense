 ...
 * A CustomPhraseQuery is built by QueryParser for input like <code>"new york"</code>.
 * 
...
public class CustomPhraseQuery extends Query {
  ...
  private int slop = 0;
  private final TermDocsFactory termDocsFactory;
  ...
  private class PhraseWeight extends Weight {
    ...
    public PhraseWeight(Searcher searcher, TermDocsFactory termDocsFactory)
      throws IOException {
    ...
    @Override
    public String toString() { return "weight(" + CustomPhraseQuery.this + ")"; }

    @Override
    public Query getQuery() { return CustomPhraseQuery.this; }
    ...
    public Scorer scorer(IndexReader reader, boolean scoreDocsInOrder, boolean topScorer) throws IOException {
      ...
      PostingsAndFreq[] cpostingsFreqs = new PostingsAndFreq[terms.size()];
      for (int i = 0; i < terms.size(); i++) {
        final Term t = terms.get(i);
        TermPositions p = reader.termPositions(t);
        if (p == null)
          return null;
        TermPositions cp = reader.termPositions(t);
        if(this.termDocsFactory!=null) {
          p = (TermPositions) this.termDocsFactory.wrapTermDocs(p);
          cp = (TermPositions) this.termDocsFactory.wrapTermDocs(cp);
        }
        postingsFreqs[i] = new PostingsAndFreq(p, reader.docFreq(t), positions.get(i).intValue(), t);
        cpostingsFreqs[i] = new PostingsAndFreq(cp, reader.docFreq(t), positions.get(i).intValue(), t);
      }
      ...
      if (slop == 0) {				  // optimize exact case
        CustomExactPhraseScorer s = new CustomExactPhraseScorer(this, postingsFreqs, cpostingsFreqs, similarity,
                                                    reader.norms(field));
        if (s.noDocs) {
          return null;
        } else {
        ...
        return
          new CustomSloppyPhraseScorer(this, postingsFreqs, cpostingsFreqs, similarity, slop,
                                 reader.norms(field));
      }
    }
  ...
  public Weight createWeight(Searcher searcher) throws IOException {
    ...
    return new PhraseWeight(searcher, termDocsFactory);
  }
