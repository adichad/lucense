...
final class CustomTermScorer extends CustomScorer {
   ...
   * @throws Exception 
   */
  CustomTermScorer(Weight weight, Similarity similarity, String field,
      int queryPos, Term term, TermDocsFactory termDocsFactory,
    ...
    this.reader = reader;
    this.termDocs = getTermDocs();
    //this.termPositions = getTermPositions();

    this.boostValue = weight.getQuery().getBoost();
  }

  private TermDocs getTermDocs() throws IOException {
    TermDocs termDocs;
    if (termDocsFactory == null
        || termDocsFactory instanceof DefaultTermDocsFactory) {
      termDocs = reader.termDocs(this.term);
    } else {
      termDocs = reader.termPositions(this.term);
    }
  ...
  private TermPositions getTermPositions() throws IOException {
    TermPositions termPositions = reader.termPositions(this.term);
    if (termDocsFactory != null)
  ...
  public void matchedTermPositions(Map<String, MatchTermPositions> mtp,
    ...
    // System.out.println("field: "+field+", scoreFields: "+scoreFields);
    if (scoreFields.contains(field)) {
      ...
      pos.add(this.queryPos, getTermPositions(), this.term);
    }
  }
}
