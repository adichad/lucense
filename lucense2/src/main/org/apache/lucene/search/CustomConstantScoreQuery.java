...
public class CustomConstantScoreQuery extends CustomQuery {
  ...
  public CustomConstantScoreQuery(Query query) {
    if (query == null)
   ...
   * {@code new CustomConstantScoreQuery(new QueryWrapperFilter(query))}, instead
   * use {@link #CustomConstantScoreQuery(Query)}!
   */
  ...
  public Query rewrite(IndexReader reader) throws IOException {
    if (query != null) {
      ...
      if (rewritten != query) {
        rewritten = new CustomConstantScoreQuery(rewritten);
  ...
  protected class ConstantWeight extends Weight {
    ...
    public Explanation explain(IndexReader reader, int doc) throws IOException {
      ...
      if (exists) {
        result.setDescription(CustomConstantScoreQuery.this.toString() + ", product of:");
        result.setValue(queryWeight);
        result.setMatch(Boolean.TRUE);
        result.addDetail(new Explanation(getBoost(), "boost"));
        result.addDetail(new Explanation(queryNorm, "queryNorm"));
      } else {
        result.setDescription(CustomConstantScoreQuery.this.toString() + " doesn't match id " + doc);
        result.setValue(0);
  ...
  public boolean equals(Object o) {
    ...
    if (o instanceof CustomConstantScoreQuery) {
      final CustomConstantScoreQuery other = (CustomConstantScoreQuery) o;
      return 
