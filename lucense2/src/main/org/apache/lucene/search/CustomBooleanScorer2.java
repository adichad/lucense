...
class CustomBooleanScorer2 extends CustomScorer {
   ...
   */
  public CustomBooleanScorer2(Similarity similarity, int minNrShouldMatch, List<Scorer> required,
      List<Scorer> prohibited, List<Scorer> optional, float boost) throws IOException {
    super(similarity);
    this.boost = boost;
  ...
  private CustomScorer countingDisjunctionSumScorer(final List<Scorer> scorers, int minNrShouldMatch)
      throws IOException {
  ...
  private CustomScorer countingConjunctionSumScorer(List<Scorer> requiredScorers) throws IOException {
    // each scorer from the list counted as a single matcher
