 ...
 * 
 */
public class AddToAuxRequest extends Request implements Runnable {
  ...
  private int boolByPassDictionary = 0;

  private long timeTaken;

  private boolean requestServed;

  public AddToAuxRequest(Socket sock, int version, int id, byte type) {
  ...
  public void run() {
    try {
      ...
      if (this.searchResourceManager.checkIncrementConcurrancy()) {
        ...
        BitMapOperationStatus os = indexer.getRowHandler().put(rowId, cs, null);
        AddToAuxResult res = new AddToAuxResult(this.id, os, commandType);
  ...
  public String toString() {
    ...
    buff.append(" sec ");
    buff.append("[rowwise-auxadd] [").append(indexName).append("] [").append(new String(rowId)).append("] [")
