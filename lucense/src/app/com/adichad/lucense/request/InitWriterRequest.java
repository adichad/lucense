...
import com.adichad.lucense.result.InitWriterResult;
 ...
 * 
 */
public class InitWriterRequest extends Request implements Runnable {
  ...
  public InitWriterRequest(Socket sock, int version, int id) {
  ...
  public void run() {
    try {
      ...
      this.searchResourceManager.openWriter(indexName);
      Result result = new InitWriterResult(this.id);
      result.writeTo(this.sock.getOutputStream());
      
      statusLogger.log(Level.INFO, "Index " + indexName + " opened successfully");
    } catch (Throwable e) {
