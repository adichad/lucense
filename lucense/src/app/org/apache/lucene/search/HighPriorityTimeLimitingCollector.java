...
/**
 * The {@link HighPriorityTimeLimitingCollector} is used to timeout search
 * requests that take longer than the maximum allowed search time limit. After
 * this time is exceeded, the search thread is stopped by throwing a
 * {@link TimeExceededException}.
 */
public class HighPriorityTimeLimitingCollector extends Collector {
  ...
  private static final class TimerThread extends Thread {
     ...
     * that they can count elapsed time with less overhead than repeatedly
     * calling System.currentTimeMillis. A single thread should be created to be
  ...
  static {
    TIMER_THREAD.start();
    TIMER_THREAD.setPriority(Thread.MAX_PRIORITY);
  }
   ...
   * @param timeAllowed
   *          max time allowed for collecting hits after which
   *          {@link TimeExceededException} is thrown
   */
  public HighPriorityTimeLimitingCollector(final Collector collector, final long timeAllowed) {
  ...
  /**
   * Set the timer resolution. The default timer resolution is 20 milliseconds.
   * This means that a search required to take no longer than 800 milliseconds
   * may be stopped after 780 to 820 milliseconds. <br>
   * Note that:
   * <ul>
   ...
   * modified to 5 milliseconds.</li>
   * <li>Setting resolution smaller than current resolution might take effect
   * only after current resolution. (Assume current resolution of 20
   * milliseconds is modified to 5 milliseconds, then it can take up to 20
   * milliseconds for the change to have effect.</li>
   * </ul>
   */
  public static void setResolution(long newResolution) {
    resolution = Math.max(newResolution, 5); // 5 milliseconds is about the
                                             // minimum reasonable time for a
   ...
   * {@link TimeExceededException} without allowing the wrapped collector to
   * collect current doc. A greedy one would first allow the wrapped hit
   * collector to collect current doc and only then throw a
   * {@link TimeExceededException}.
   * 
